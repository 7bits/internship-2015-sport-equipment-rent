...
// Проверка на пустоту полей ввода цен на странице подачи объявления
public class _10676_06_00 extends TestCase{
  ...
  public void test106760600() throws Exception {
    ...
    try {
      assertEquals("Поле цена за час не может быть пустым", driver.findElement(By.cssSelector("li > p")).getText());
    } catch (Error e) {
      verificationErrors.append(e.toString());
    }
    try {
      assertEquals("Поле цена за день не может быть пустым", driver.findElement(By.xpath("//li[2]/p")).getText());
    } catch (Error e) {
      verificationErrors.append(e.toString());
    }
    try {
      assertEquals("Поле цена за неделю не может быть пустым", driver.findElement(By.xpath("//li[3]/p")).getText());
    } catch (Error e) {
      verificationErrors.append(e.toString());
    }
  }
