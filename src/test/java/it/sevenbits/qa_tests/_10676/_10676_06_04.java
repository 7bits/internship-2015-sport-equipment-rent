...
// Проверка на ввод десятичных дробей в поля цен на странице подачи объявления
public class _10676_06_04 extends TestCase{
  ...
  @Test
  public void test106760604() throws Exception {
    ...
    driver.findElement(By.id("PriceForHour")).sendKeys("100.0");
    driver.findElement(By.id("PriceForDay")).clear();
    driver.findElement(By.id("PriceForDay")).sendKeys("200.5");
    driver.findElement(By.id("PriceForWeek")).clear();
    driver.findElement(By.id("PriceForWeek")).sendKeys("500.3");
    try {
    ...
    try {
      assertEquals("Поле цена за час должно содержать только цифры", driver.findElement(By.cssSelector("li > p")).getText());
    } catch (Error e) {
      verificationErrors.append(e.toString());
    }
    try {
      assertEquals("Поле цена за день должно содержать только цифры", driver.findElement(By.xpath("//li[2]/p")).getText());
    } catch (Error e) {
      verificationErrors.append(e.toString());
    }
    try {
      assertEquals("Поле цена за неделю должно содержать только цифры", driver.findElement(By.xpath("//li[3]/p")).getText());
    } catch (Error e) {
      verificationErrors.append(e.toString());
    }
  }
