...
// Проверка на ввод в поля цены 6-значных чисел
public class _10676_06_05 extends TestCase{
  ...
  public void test106760605() throws Exception {
    ...
    try {
      ...
      driver.findElement(By.id("PriceForHour")).sendKeys("100000");
      driver.findElement(By.id("PriceForDay")).clear();
      driver.findElement(By.id("PriceForDay")).sendKeys("200000");
      driver.findElement(By.id("PriceForWeek")).clear();
      driver.findElement(By.id("PriceForWeek")).sendKeys("500000");
    try {
    ...
    try {
      assertEquals("Поле цена за час может содержать не более 5 цифр", driver.findElement(By.cssSelector("li > p")).getText());
    } catch (Error e) {
      verificationErrors.append(e.toString());
    }
    try {
      assertEquals("Поле цена за день может содержать не более 5 цифр", driver.findElement(By.xpath("//li[2]/p")).getText());
    } catch (Error e) {
      verificationErrors.append(e.toString());
    }
    try {
      assertEquals("Поле цена за неделю может содержать не более 5 цифр", driver.findElement(By.xpath("//li[3]/p")).getText());
    } catch (Error e) {
      verificationErrors.append(e.toString());
    }
  }
