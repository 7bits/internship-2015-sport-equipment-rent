...
// Проверка на ввод в поля "Цена..." чисел, содержащих более 5 знаков
public class _10676_06_05 extends TestCase{
  ...
  @Test
  public void test106760605() throws Exception {
    ...
    driver.findElement(By.id("PriceForHour")).sendKeys("100500");
    driver.findElement(By.id("PriceForDay")).clear();
    driver.findElement(By.id("PriceForDay")).sendKeys("200600");
    driver.findElement(By.id("PriceForWeek")).clear();
    driver.findElement(By.id("PriceForWeek")).sendKeys("300700");
    ...
    try {
      assertEquals("Поле Цена за час может содержать не более 5 цифр", driver.findElement(By.cssSelector("p")).getText());
    } catch (Error e) {
      verificationErrors.append(e.toString());
    }
    try {
      assertEquals("Поле Цена за сутки может содержать не более 5 цифр", driver.findElement(By.cssSelector("p")).getText());
    } catch (Error e) {
      verificationErrors.append(e.toString());
    }
    try {
      assertEquals("Поле Цена за неделю может содержать не более 5 цифр", driver.findElement(By.cssSelector("p")).getText());
    } catch (Error e) {
      verificationErrors.append(e.toString());
    }
  }
