...
// Проверка на ввод в поля описания 2049 латинских букв на странице подачи объявления
public class _10676_05_01 extends TestCase{
  ...
  @Test
  public void test106760501() throws Exception {
    ...
    driver.findElement(By.id("announcementsDescription")).sendKeys(
            ...
            "test test test test test test test test test test test test test test test test" +
            "test test test test test test test test test test test test test test test test" +
            "test test test test test test test test test test test test test test test test" + "t"
    );
    try {
    ...
    try {
      assertEquals("Поле описание не может содержать больше 2048 символов", driver.findElement(By.cssSelector("")).getText());
    } catch (Error e) {
      verificationErrors.append(e.toString());
    }
  }
